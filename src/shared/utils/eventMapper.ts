...
export function mapDbEventToApp(db: any): Event {
  return {
    ...
    showFeeToBuyer: db.show_fee_to_buyer ?? db.showFeeToBuyer ?? false,
...
export function mapAppEventToDb(evt: Event): any {
  return {
    ...
    total_bistros: evt.tableConfig?.totalBistros,
    table_price: evt.tableConfig?.tablePrice,
    bistro_price: evt.tableConfig?.bistroPrice,
    table_icon_size: evt.tableConfig?.globalIconSize,
    ...
    show_fee_to_buyer: evt.showFeeToBuyer ?? false,
